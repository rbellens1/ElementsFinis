...
#     print(mesh)
#     uv = np.loadtxt("data/outfile.txt", skiprows=1, delimiter=",")
...
if __name__ == "__main__":
    ...
    # Chargement du maillage et des déplacements
    mesh = Mesh("data/mesh.txt")
    uv = np.loadtxt("data/result.txt", skiprows=1, delimiter=",")
    uv_norm = np.linalg.norm(uv, axis=1)
    factor = 1e+7
    ...
    def animate(frame):
        plt.clf()  # Clear la figure
        plt.gca().set_aspect("equal")
        plt.grid(alpha=0.2)
        
        # Calcul de la déformation interpolée
        t = frame / 100  # 100 frames pour l'animation complète
        current_deformation = uv * factor * t
        
        # Calcul de la norme de la déformation courante
        current_norm = np.linalg.norm(current_deformation.reshape(-1, 2), axis=1)
        
        # Plot du champ de déformation avec la norme courante
        cb = mesh.plotfield(current_norm, current_deformation, cmap="turbo")
        plt.colorbar(cb)
        mesh.plot(current_deformation, lw=0.2, c="k")
        
        plt.title(f'Déformation: {t*100:.0f}%')

    # Création de l'animation
    anim = animation.FuncAnimation(fig, animate, frames=100, interval=50, blit=False)
    
    # Sauvegarde de l'animation
    anim.save('deformation.gif', writer='pillow')
